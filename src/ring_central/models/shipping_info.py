...
class ShippingInfo(BaseModel):
    ...
    :param status: Order item shipping status. It is set to `Initial` when the order is submitted. Then it is changed to `Accepted` when a distributor starts processing the order. Finally, it is changed to `Shipped` which means that the distributor has shipped the device. , defaults to None
    :type status: ShippingStatus, optional
    :param carrier: Shipping carrier name. Appears only if the device status is 'Shipped' , defaults to None
    :type carrier: str, optional
    :param tracking_number: Carrier-specific tracking number. Appears only if the device status is 'Shipped' , defaults to None
    :type tracking_number: str, optional
    :param method: method, defaults to None
    :type method: ShippingMethodInfo, optional
    :param address: Shipping address for the order. If it coincides with the Emergency Service Address, then can be omitted. By default, the same value as the emergencyServiceAddress. Multiple addresses can be specified; in case an order contains several devices, they can be delivered to different addresses , defaults to None
