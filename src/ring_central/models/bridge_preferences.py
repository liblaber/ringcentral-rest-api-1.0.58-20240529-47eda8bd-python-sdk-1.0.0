...
class BridgePreferences(BaseModel):
    ...
    :param play_tones: Specifies enter and exit tones mode. 1) On - Enter and exit tones switched on. 2) Off - Enter and exit tones switched off. 3) ExitOnly - Only exit tones switched on. 4) EnterOnly - Only enter tones switched on. , defaults to None
    ...
    :param join_before_host: Specifies if participants can join to a meeting before host. Default value depends on the bridge type: - default bridge (PMI) -> false - other types -> true , defaults to None
