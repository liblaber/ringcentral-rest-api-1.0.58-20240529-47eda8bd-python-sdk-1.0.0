...
class QueueInfo(BaseModel):
    ...
    :param hold_time_expiration_action: Specifies the type of action to be taken after the hold time (waiting for an available call queue member) expires. If 'TransferToExtension' option is selected, the extension specified in `transfer` field is used. The default value is `Voicemail` , defaults to None
    ...
    :param wrap_up_time: Minimum post-call wrap up time in seconds before agent status is automatically set; the value range is from 0 to 300 sec. , defaults to None
