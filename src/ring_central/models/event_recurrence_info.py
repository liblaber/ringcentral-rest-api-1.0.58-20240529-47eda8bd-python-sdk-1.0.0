...
class EventRecurrenceInfo(BaseModel):
    ...
    :param schedule: Event recurrence settings. For non-periodic events the value  is `None`. Must be greater or equal to event duration: 1- Day/Weekday;  7 - Week; 28 - Month; 365 - Year , defaults to None
    ...
    :param ending_after: Count of event iterations. For periodic events only. Value range is 1 - 10.  Must be specified if `endingCondition` is `Count` , defaults to None
    :type ending_after: int, optional
    :param ending_on: Iterations ending datetime for periodic events in  [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format , defaults to None
