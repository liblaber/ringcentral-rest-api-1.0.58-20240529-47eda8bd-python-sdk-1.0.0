...
class PubNubDeliveryMode(BaseModel):
    ...
    :param encryption: Optional. Specifies if notification messages will be encrypted or not. Please note that for some event filters (e.g. presence) encryption is mandatory and `false` value provided by caller will be ignored.
    :type encryption: bool
    :param address: PubNub channel name
    :type address: str
    ...
    :type secret_key: str
    :param encryption_algorithm: (Only for a "PubNub" transport, returned only if `encryption` is `true`) Encryption algorithm used , defaults to None
    :type encryption_algorithm: EncryptionAlgorithm, optional
    :param encryption_key: (Only for a "PubNub" transport, returned only if `encryption` is `true`) Cryptographic key to decrypt PubNub notification messages , defaults to None
