...
class MessageStoreCallerInfoResponseTo(BaseModel):
    ...
    :param extension_number: Extension short number (usually 3 or 4 digits). This property is filled when parties communicate by means of short internal numbers, for example when calling to other extension or sending/receiving Company Pager messages , defaults to None
    :type extension_number: str, optional
    :param extension_id: extension_id, defaults to None
    :type extension_id: str, optional
    :param location: Contains party location (city, state) if one can be determined from a phone number. This property is filled only when `phoneNumber` is not empty and a server can calculate location information from it (for example, this information is unavailable for US toll-free numbers) , defaults to None
    :type location: str, optional
    :param target: The `true` value specifies that message is sent exactly to this recipient. Returned in the `to` field for group MMS. Useful if one extension has several phone numbers' , defaults to None
